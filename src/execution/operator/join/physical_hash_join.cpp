...
// Multilevel Bloom Filter Integration
...
// Flag to enable/disable multilevel bloom filter
static constexpr bool enable_multilevel_bloom_filter = true;
static constexpr bool force_disable_perfect_hash = true; // new
...
public:
	...
	// Bloom filter object
	// unique_ptr<MultilevelBloomFilter> bloom_filter;
...
SinkResultType PhysicalHashJoin::Sink(ExecutionContext &context, DataChunk &chunk, OperatorSinkInput &input) const {
	...
	// If enabled, populate Bloom filter with join keys
	if (enable_multilevel_bloom_filter && !gstate.bloom_filter) {
		// gstate.bloom_filter = make_uniq<MultilevelBloomFilter>(3, 10000, 3, 5); // customize params as needed
		// gstate.bloom_filter = make_uniq<BloomFilter>(15, 3);
		gstate.bloom_filter = make_uniq<BloomFilter>(150000, 3);
	}
	if (enable_multilevel_bloom_filter) {
		for (idx_t i = 0; i < lstate.join_keys.size(); i++) {
			auto key = lstate.join_keys.data[0].GetValue(i).ToString();
			// gstate.bloom_filter->Query(key); //multilevel
			gstate.bloom_filter->Insert(key);
			// auto key = lstate.join_keys.data[0].GetValue(i).Hash();
			// gstate.bloom_filter->Insert(key);
			// auto key_val = lstate.join_keys.data[0].GetValue(i);
			// auto key_hash = duckdb::Hash(key_val);
			// gstate.bloom_filter->Insert(key_hash);
...
SinkFinalizeType PhysicalHashJoin::Finalize(Pipeline &pipeline, Event &event, ClientContext &context,
	...
	// new
	auto use_perfect_hash = !force_disable_perfect_hash &&
...
OperatorResultType PhysicalHashJoin::ExecuteInternal(ExecutionContext &context, DataChunk &input, DataChunk &chunk,
	...
	// Apply bloom filter on probe keys
	// if (enable_multilevel_bloom_filter && sink.bloom_filter) {
	// 	SelectionVector sel(input.size());
	// 	idx_t sel_size = 0;
	// 	for (idx_t i = 0; i < state.lhs_join_keys.size(); i++) {
	// 		auto key = state.lhs_join_keys.data[0].GetValue(i).ToString();
	// 		// if (sink.bloom_filter->PassesAllLevels(key)) {
	// 		if (sink.bloom_filter->PossiblyContains(key)) {
	// 			sel.set_index(sel_size++, i);
	// 		}
	// 	}
	// 	state.lhs_join_keys.Slice(sel, sel_size);
	// 	input.Slice(sel, sel_size);
	// }
	...
	// 	std::cout << "input[" << i << "]: " << input.GetValue(0, i).ToString() << std::endl;
	// }
	
	if (state.scan_structure.is_null) {
		// probe the HT, start by resolving the join keys for the left chunk
		state.lhs_join_keys.Reset();
		// std::cout << "input size before executor: " << input.size() << std::endl;
		// std::cout << "Join keys before Bloom: " << state.lhs_join_keys.size() << std::endl;
		state.probe_executor.Execute(input, state.lhs_join_keys);
		// std::cout << "input size after executor: " << input.size() << std::endl;
		// Apply bloom filter on probe keys
		if (enable_multilevel_bloom_filter && sink.bloom_filter) {
			SelectionVector sel(input.size());
			idx_t sel_size = 0;
			...
			state.lhs_join_keys.Slice(sel, sel_size);
			input.Slice(sel, sel_size);
			// std::cout << "Join keys after Bloom: " << state.lhs_join_keys.size() << std::endl;
			// std::cout << "input after Bloom: " << input.size() << std::endl;
		}
